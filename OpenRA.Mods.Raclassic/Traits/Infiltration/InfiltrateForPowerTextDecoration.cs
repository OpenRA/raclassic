...
{
	[Desc("Reveals the generated and used power of the owner to the indicated players when infiltrated.")]
	sealed class InfiltrateForPowerTextDecorationInfo : WithPowerTextDecorationInfo
	{
		...
		public override object Create(ActorInitializer init) { return new InfiltrateForPowerTextDecoration(init.Self, this); }
	}

	sealed class InfiltrateForPowerTextDecoration : WithPowerTextDecoration, INotifyInfiltrated
	{
		readonly HashSet<Player> infiltrators = [];
		readonly InfiltrateForPowerTextDecorationInfo info;

		public InfiltrateForPowerTextDecoration(Actor self, InfiltrateForPowerTextDecorationInfo info)
			: base(self, info)
		{
			this.info = info;
		}
